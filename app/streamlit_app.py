...
def reset_analysis_state() -> None:
    ...
    st.session_state.analysis_mode = "快速预览"
    st.rerun()
...
def classify_analysis_error(exc: Exception) -> dict[str, Any]:
    ...
    if isinstance(exc, MissingAPIKeyError) or "api key" in lower or "deepseek_api_key" in lower:
        return {
            "message": "当前站点尚未完成分析服务配置，暂时无法生成自动分析。",
            "suggestions": [
                "如果你是站点维护者，请检查部署平台中的服务密钥配置。",
                "如果你只是使用者，可以稍后刷新页面或联系站点维护者。",
            ],
        }
    ...
    if "timeout" in lower or "超时" in message:
        return {
            "message": "AI 分析服务响应超时，本次分析没有在限定时间内完成。",
            "suggestions": [
                "建议先使用快速分析和快速预览模式。",
                "稍后重试，或减少证据数量。",
    ...
    if "sentence" in lower or "embedding" in lower or "torch" in lower or "bge" in lower:
        return {
            "message": "本地分析组件加载失败，暂时无法完成原文证据提取。",
            "suggestions": [
                "本地运行时请确认依赖已安装完整。",
                "线上首次打开可能较慢，可稍后刷新重试。",
            ],
    ...
    if "deepseek" in lower or "调用失败" in message:
        return {
            "message": "AI 分析服务调用失败，可能是网络波动或服务繁忙。",
            "suggestions": [
                "可以先切换到快速分析。",
                "稍后重试一次。",
            ],
...
def set_running_state(
    *,
    ...
    st.session_state.analysis_mode = "快速预览" if quick_test_mode else "完整分析"
...
def render_running_panel() -> dict[str, Any]:
    ...
    mode = st.session_state.get("analysis_mode") or "快速预览"
    estimated_time = st.session_state.get("estimated_time") or "正在估算"
    cleaning_mode = "加强正文整理" if st.session_state.get("use_enhanced_cleaning") else "标准正文整理"

    st.markdown(
        f"""
          <div class="running-panel">
            <div class="section-title">正在分析你的论文</div>
          <div class="subtle-text">系统正在整理正文、提取原文证据并生成思想谱系报告。</div>
          <div style="margin-top: 12px;">
            <span class="status-pill">当前文件：{file_name}</span>
            <span class="status-pill">分析模式：{model_display_name(model)}</span>
            <span class="status-pill">分析范围：{mode}</span>
            <span class="status-pill">正文整理：{cleaning_mode}</span>
    ...
    st.markdown(
        ...
        <div class="step-card">
          <div class="summary-label">分析流程</div>
            ...
            <div class="step-item">正文整理</div>
            <div class="step-item">证据片段划分</div>
            <div class="step-item">原文证据提取</div>
            <div class="step-item">标签判断</div>
...
def render_upload_card(label_definitions: list[dict[str, Any]]) -> None:
    ...
    with st.container(border=True):
        ...
        if analysis_status == "running":
            ...
            st.caption(f"分析模式：{model_display_name(st.session_state.get('selected_model_name') or get_deepseek_model())}")
        ...
        if analysis_status == "done" and st.session_state.get("analysis_result"):
            ...
            st.caption(f"当前文件：{st.session_state.get('uploaded_file_name') or '已上传论文'}")
            st.caption(f"分析模式：{model_display_name(st.session_state.get('analysis_result', {}).get('model') or get_deepseek_model())}")
            if st.button("重新上传论文", width="stretch"):
                reset_analysis_state()

        if analysis_status == "error":
            st.error("上一次分析失败。请检查文件格式或稍后重新分析。")
        ...
        st.caption("快速分析：适合快速预览，耗时较短。深度分析：适合正式报告，分析更完整。")
        quick_test_mode = st.checkbox(
            "快速预览模式",
            value=True,
            help="只分析少量核心维度，用于快速了解论文的大致倾向；正式分析可关闭该模式。",
        )
        use_enhanced_cleaning = st.checkbox(
            "启用加强正文整理（实验）",
            value=False,
            help=(
                "启用后，系统会在分析前对 PDF 提取文本进行格式整理与结构识别，"
        ...
        if quick_test_mode:
            st.caption("快速预览会优先分析：现代性批判、思想史研究、社会历史批评、历史唯物主义、启蒙理性。")
        if use_enhanced_cleaning:
            st.caption("加强正文整理会尽量处理页眉页脚、脚注、参考文献和乱码干扰；如整理失败，会自动回到原始正文。")
        core_only = st.toggle("证据链只展示分数 >= 2 的核心标签", value=True)
        key_status, key_message = get_deepseek_key_status()
        if key_status == "invalid":
            st.error("分析服务配置异常，请联系站点维护者。")
        elif key_status != "configured":
            st.warning("当前站点暂未配置分析服务，无法生成报告。")
        base_estimated_time = estimate_analysis_time(selected_model_name, quick_test_mode, 0)
        mode_name = "快速预览" if quick_test_mode else "完整分析"
        st.info(
        ...
        st.caption(
            "隐私提示：上传论文仅用于本次分析；请勿上传涉密、未授权或不希望被第三方模型处理的文本。"
        )
...
def render_compact_analysis_summary(status: str) -> None:
    ...
    mode = st.session_state.get("analysis_mode") or ("快速预览" if result.get("quick_test_mode") else "完整分析")
    status_text = {
        "running": "分析进行中",
        "done": "分析已完成",
        "error": "等待重新分析",
    ...
    st.markdown(
        ...
        <div class="progress-card">
          ...
          <div class="subtle-text">分析模式：{html_lib.escape(model_display_name(str(model)))}</div>
          <div class="subtle-text">分析范围：{html_lib.escape(str(mode))}</div>
...
def render_result_summary_card(result: dict[str, Any]) -> None:
    ...
    st.markdown(
        ...
        <div class="summary-card">
          ...
          <div class="subtle-text" style="margin-top: 8px;">已生成核心判断、学术谱系定位、原文证据和结构化报告。</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_export_tab(result: dict[str, Any]) -> None:
    ...
    st.markdown(
        ...
        <div class="export-card">
          ...
          <div class="subtle-text">可下载完整报告，也可保留一份分析数据用于后续复核。</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        log_event(
            "analysis_completed",
            {
                ...
                "analysis_mode": "快速预览" if pending.get("quick_test_mode") else "完整分析",
