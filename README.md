...
系统会在 RAG 分析前保留 `raw_text`，并生成用于 chunk 切分和证据召回的 `cleaned_text`。PDF 会同时尝试 PyMuPDF 和 pdfplumber 两种解析后端，分别计算文本质量分，并自动选择质量更高的提取结果。

清洗流程只做格式处理和结构识别，包括删除 Unicode 私有区乱码、修复明显 PDF 断行、去除孤立页码、过滤 CNKI 页脚/DOI/期刊页眉、分离注释和参考文献、标记不确定片段。参考文献和注释会保留在 metadata 中，但默认不进入正文 chunk 和 RAG 召回。

上传页提供“启用增强文本清洗（实验）”开关，默认关闭。开启后，大模型只辅助做格式清洗和结构识别，不会总结、改写或补充论文内容；如果清洗失败，会自动回退到规则清洗或原始文本。

结果页会显示清洗诊断摘要，包括 PDF 解析后端、原始/清洗后质量分、删除噪声数量、识别参考文献字数、进入 RAG 的 chunk 数量和过滤 chunk 数量；不会展示完整 `raw_text` 或把用户论文全文写入固定文件。
