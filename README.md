...
上传页提供“启用增强文本清洗（实验）”开关，默认关闭。开启后，系统会在 RAG 分析前保留 `raw_text`，并生成用于 chunk 切分和证据召回的 `cleaned_text`。

清洗流程只做格式处理和结构识别，包括修复明显 PDF 断行、去除孤立页码、识别页眉页脚、标记疑似脚注/参考文献和不确定片段。大模型辅助清洗不会总结、改写或补充论文内容；如果清洗失败，会自动回退到规则清洗或原始文本。

结果页只显示清洗摘要，包括清洗方式、原始文本字数、清洗后字数、删除噪声数量、疑似脚注/参考文献数量和 warnings 数量；不会展示完整 `raw_text` 或把用户论文全文写入固定文件。
